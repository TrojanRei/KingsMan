...
function validateForm() {
    ...
    if (orderSummary.children.length === 0) {
        alert("Order summary must not be empty");
        return false; // Prevent form submission
    } else {
...
function generateReceipt(name, address, contact) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        // Extract the total price for each product from the third cell and add it to the totalOrderPrice
        var totalPriceCell = cells[2];
        var totalPrice = parseFloat(totalPriceCell.innerText.replace('Total: $', ''));
        totalOrderPrice += totalPrice;

        receiptContent += "\n";
    }

    receiptContent += "\nTotal Price of All Items: $" + totalOrderPrice.toFixed(2);
